...
## 2026-08-19 — nibble table (table id 51)

Ported from the `pivco_nibble` OpenZL experiment (#30)
...
The fix is one extra table id, `PIVCO_FSE_NIBBLE_ID` (= 51)
Instead of a pre-built distribution it means:
...
cannot pay for itself, a 16-symbol one at `tableLog` ≤ 7
...
- **nibble** — no skew precondition (a near-50/50 bitmap can
  still have a lopsided nibble histogram), no xor flip.

The comparison is header-inclusive for free: the length the
nibble path reports already contains its table description, and
`pivco_fse_compress_nibble` returns `FALLBACK` outright when
...
Never worse, by construction — the nibble form only lands when
...
Four columns — pivco (FSE off), pivco+static-ANS, pivco+nibANS,
...
static-vs-nibble split is driven by `pivco_cfg_t.fse_nibble_enabled`
...
text (`+0.00%` vs plain pivco) while the nibble table carries
essentially the whole FSE win — calgary_pic −38.0%, csv_numeric
−6.4%, source_c −1.6%, README.md −1.8%.  Aggregate over the 13
dataset files: pivco 58.96% of raw, +static 58.19%, +nibANS 57.70%,
fse 57.73%.
...
- reuse the `fse_xy_codec.h` wide-cursor decoder — it needs a
  fast-mode-safety check against the *nibble* table, computed
...
## Nibble-path design choices: flat-region coding and per-candidate gates

Two coding choices make the nibble path pay off on literal streams.
Both come from the reference `pivco_nibble` kernel
(`encode_pivco_nibble_kernel.c`); an early port omitted them and lost to
stock FSE on literal streams (`nci.serial`, `xml.serial`) as a result.

### Flat-subtree regions are coded, not emitted raw

Every bitmap goes through the FSE attempt — including a flat leaf's
`n·D` packed bits, not just internal nodes' partition bitmaps.  That is
where the redundancy sits: a depth-D flat subtree assigns all 2^D of its
...
The static tables commit on a per-codeword **speed** gate
(`(depth + fse_frac) / (depth + 1) <= MIN_RATIO`): stock-table FSE
decode is slow enough that a marginal byte saving is a bad trade.  That
gate is wrong for the nibble table — the ratio tends to 1 as depth
grows no matter how many bytes the coding saves, so it would decline
most deep nodes.

Each candidate therefore carries its own gate: static keeps the
per-codeword one; the nibble table commits on any header-inclusive gain
(`fse_len + 2 < nbytes`).  On nci that is the difference between 8 and
216 committed nibble records per file.

### Bench accounting: the code-length table is charged entropy-coded

`pivco_bench_chunk_sizes` charges the 128-byte nibble-packed code-length
table as `1 + min(128, FSE(table))`, not raw — FSE's own table
description is entropy-coded inside `FSE_compress`'s output, and huf0
compresses its weights for the same reason.  Tables are per-chunk by
default (the policy `FSE_compress` uses, so both sides adapt at the same
rate); `--table per-file` matches what `pivcohuf_file.c` and the openzl
codec do (one weights stream per input).  Per-file is much worse on
strongly non-stationary streams — `xml.serial` 251947 vs 238823 bytes,
~5% — which is why per-chunk is the default.

### Measured

`pivco_bench_chunk_sizes`, 32 KiB chunks, header-inclusive:

| file | raw flats, speed gate | coded flats, gain gate | fse | raw vs fse | coded vs fse |
|---|---:|---:|---:|---:|---:|
...
Broader sweep, `extras/datasets/` plus the literal corpus: nibANS now beats
...
Where the design lands against that floor:

| file | entropy O0 | pivco | +sANS | +nibANS | fse |
|---|---:|---:|---:|---:|---:|
...
So nibANS is under the order-0 floor in aggregate and plain FSE is not,
...
- **Nibble tableLog.**  The source uses stock `FSE_compress` (default
  tableLog 11).  Swept 6–12: the spread over the two files is 55 bytes
  out of 704k, so it is not where the ratio went.  The encoder caps at 7
  (decode +16–41% on the fleet from the smaller per-record build) and
  the decoder accepts up to 10 (`PIVCO_FSE_NIB_TABLELOG` / `_MAX`).

### Cost

Decode throughput on the main bench falls hard now that flat regions —
the bulk of the data — go through the nibble path: html_wiki 6576 →
...
`pivco_cfg_t.fse_nibble_enabled = 0` restores the previous speed and the
...
nibble table on (53958376 over the 31-file corpus, was 54118800 under
vertical).  `test_flat_layout_file` asserts that equality so a
regression cannot pass silently.

| file | vertical | natural | fse | verdict |
|---|---:|---:|---:|---|
