...
distributions, see `extras/bench/bench_flat_subtree_stats.c`):
...
5. **Per-node FSE coding of partition bitmaps** is implemented
   ...
   `extras/bench/bench_fse_xy_micro.c` shows that FSE's stock x=2-state
...
state-per-instruction walkthroughs, see [`docs/KERNELS.md`](docs/KERNELS.md).
...
metadata below is generated by [`extras/bench/bench_dist_stats.c`](extras/bench/bench_dist_stats.c)
...
`extras/bench/bench_flat_optimal.c` predicted 16-27% partition-step
...
- **Flat-subtree fast path** (April 2026, commits `a275d05` /
  ...
  **Headlines on M4 (pivco_n, M/s):**
  ...
  full 4-platform sweep; `extras/bench/bench_flat_subtree_stats.c` is the
...
- **Nested (multi-stage) prefix-radix**: At each internal node during
  ...
  `extras/bench/bench_multi_stage_stats.c` shows this fires on a meaningful
...
git clone --depth 1 https://github.com/google/brotli.git ext/brotli  # optional, extras/bench/bench_brotli
