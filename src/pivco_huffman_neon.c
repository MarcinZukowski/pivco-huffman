 ...
 * MSB-first as before. */
/* Per-D SIMD bit-pack helpers — symmetric to flat_dN_unpack in
 * pivco_huffman_neon_flat.h.  Each one reads D-bit codes from codes_la
 * (where bit-d of the original Huffman code is at left-aligned
 * position 15-d), shifts them right by `right_shift = 16 - depth - D`
 * to land in the low D bits, and packs them LSB-first into the output
                          ...
                          int n, int right_shift)
{
    ...
    for (; i + 16 <= n; i += 16) {
        ...
        /* Sum groups of 4 via two paired-adds.  Final low 4 lanes are
         * the 4 output bytes; lanes 4..7 are an unused duplicate. */
                          ...
                          int n, int right_shift)
{
    ...
    for (; i + 8 <= n; i += 8) {
        ...
        uint32x4_t hi = vshlq_u32(vmovl_u16(vget_high_u16(v)),
                                   vld1q_s32(shifts_hi));
        /* No bit overlap between lanes after the shifts, so ADD == OR. */
        uint32x4_t sum = vaddq_u32(lo, hi);
        uint32_t packed = vaddvq_u32(sum);
        /* Write 3 bytes (low 24 bits).  Writing 4 here would clobber a
         * byte past the end on the last group; do byte-by-byte. */
        int bi = i * 3 / 8;
                          ...
                          int n, int right_shift)
{
    ...
    for (; i + 8 <= n; i += 8) {
        ...
        /* Widen each pair of codes to uint64x2, shift, OR. */
                                      ...
                                      const uint16_t *codes_la)
{
    ...
    int i = 0;
    switch (D) {
    case 2: i = pack_d2(out, codes_la, n, right_shift); break;
    case 3: i = pack_d3(out, codes_la, n, right_shift); break;
