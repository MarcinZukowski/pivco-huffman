 ...
 * pivco_fse_tables.h.
 *
 * On top of those static tables there is an extra table id,
 * PIVCO_FSE_NIBBLE_ID, that means "nibble table": instead of a
 ...
 * alphabet does not need a big table: the per-record table build is
 ...
 * since NCount carries the log per record. */
