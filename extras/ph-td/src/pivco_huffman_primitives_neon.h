/* pivco_huffman_primitives_neon.h — NEON encode partition primitive
 * (stripped TD-slice subset of the upstream file).
 *
 * In the unify-framework era (post-2026-05-14) this header carried the
 * full primitive interface — BU decode + encode + flat-subtree pack
 * helpers — that the unified codec.c consumed.  For the TD historical
 * slice we only need the encoder's partition primitive (called from
 * encode_node_neon in pivco_huffman_neon.c).  Everything else is
 * stripped.
 */
...
#include "pivco_huffman_neon_tables.h"   /* compress_tab, compress_popcnt */
...
/* -----------------------------------------------------------------
 * Encode init + flat-subtree pack helpers (pack_d2..d8 + pack_dN).
 * Imported verbatim from the upstream pivco_huffman_primitives_neon.h
 * at SHA 31cbf75 — the TD encoder calls enc_init_neon to gather
 * left-aligned codes and pack_dN_neon at flat internal nodes.
 * ----------------------------------------------------------------- */
