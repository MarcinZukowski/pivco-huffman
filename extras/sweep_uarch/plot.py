...
def main():
    if len(sys.argv) < 2:
        print("usage: plot.py <results_dir>", file=sys.stderr); sys.exit(1)
    results_dir = sys.argv[1]
    # default dec_op: stock huf0 has no prebuilt-table API, so dec_op is
    # the only apples-to-apples column.  Use dec_pb only with huf0_4x2.
    metric = sys.argv[2] if len(sys.argv) > 2 else "dec_op"

    hosts = load_hosts()
    by_family = defaultdict(list)  # family -> list of (year, alias, uarch, ratios)
    ...
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))   # per-family y axis
    for ax, fam in zip(axes, families):
        points = sorted(by_family.get(fam, []))
        if not points:
        ...
        ax.plot(xs, ys_mean, "-o", color=c, lw=2, ms=7, label="mean across dists")
        ax.plot(xs, ys_min,  "--", color=c, lw=1, alpha=0.7)
        ax.plot(xs, ys_max,  "--", color=c, lw=1, alpha=0.7)
        for x, y, lbl in zip(xs, ys_mean, labels):
            ax.annotate(lbl, (x, y), xytext=(0,8), textcoords="offset points",
                        ha="center", fontsize=8)
        ax.axhline(1.0, color="grey", linestyle=":", linewidth=1)
        # parity label in axis-fraction coords (bottom-right, just above the line if 1.0 is at floor)
        ...
        # integer-year ticks at every measured point; abbreviate to "'YY"
        # when a tick lands within 1 year of its predecessor (avoids overlap,
        # e.g. Intel 2012/2013).
        ax.set_xticks(xs)
        tick_labels = []
    ...
    fig.suptitle("PIVCO-Huffman decode speedup over Huff0 by CPU generation",
                 fontsize=12)
    fig.tight_layout()
