/* Bottom-up vs top-down decoder A/B bench (NEON).  Same workload as
 * the main bench but reports just the two pivco decoders side-by-side. */
...
int main(int argc, char **argv) {
    ...
    printf("Bottom-up vs top-down NEON decoder (reps=%d, nblocks=%d/run)\n", reps, nblocks);
    ...
    for (int d = 0; d < bench_num_distributions(); d++) {
        ...
        /* Correctness: bu must match td on first block */
        size_t consumed;
        pivco_huffman_decode_neon(enc_buf, enc_off[1], &table,
                                   dec_buf, &consumed);
        uint8_t bu_dec[PIVCO_BLOCK_SIZE];
        pivco_huffman_decode_bu_neon(enc_buf, enc_off[1], &table,
                                      bu_dec, &consumed);
        ...
        /* Warmup */
        for (int b = 0; b < nblocks; b++) {
            pivco_huffman_decode_neon(enc_buf + enc_off[b],
        ...
        for (int r = 0; r < reps; r++) {
            for (int b = 0; b < nblocks; b++) {
                pivco_huffman_decode_bu_neon(enc_buf + enc_off[b],
                                              enc_off[b + 1] - enc_off[b],
                                              &table, dec_buf + (size_t)b * PIVCO_BLOCK_SIZE,
                                              &consumed);
            }
