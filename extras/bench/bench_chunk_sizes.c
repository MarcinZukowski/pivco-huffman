 ...
 * here beyond decode speed: the nibble table reads the packed
                   ...
                   uint64_t *grand_raw, double *grand_entropy_bits)
{
    ...
    /* pivco_encode's contract is PIVCO_MAX_ENCODED_SIZE of output room, and a
     * chunk decodes back to n <= chunk bytes.  When the chunk exceeds
     * PIVCO_BLOCK_SIZE -- as the 32 KiB default does on Apple Silicon, where
     * PIVCO_BLOCK_SIZE is 16 KiB -- the decode buffer must size to the chunk,
     * not the block, or a full chunk overruns it. */
    size_t enc_cap = FSE_compressBound(chunk);
    if (enc_cap < (size_t)PIVCO_MAX_ENCODED_SIZE) enc_cap = PIVCO_MAX_ENCODED_SIZE;
    size_t dec_cap = chunk > (size_t)PIVCO_BLOCK_SIZE ? chunk : (size_t)PIVCO_BLOCK_SIZE;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--table") == 0 && i + 1 < argc) {
            ...
            printf("Usage: %s [--chunk BYTES] [--flat LAYOUT] [--table POLICY] FILE [...]\n"
                   ...
                   "  with pivco / pivco+static-ANS / pivco+nibANS / fse and\n"
