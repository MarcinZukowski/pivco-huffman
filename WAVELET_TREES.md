...
- **Dinklage, Fischer, Kurpicz, Tarnowski — "Bit-Parallel (Compressed)
  ...
  https://www.kurpicz.org/assets/publications/dcc_2023.pdf — extends to
  AVX-512 with vpcompress and vpshufbit, including the **Huffman-shaped**
  variant. They explicitly document the same trick pivco uses:
...
- **Baruch, Klein, Shapira — "Accelerated partial decoding in wavelet trees"**
  ...
  walks the levelwise bitmaps top-down once and tracks per-position state,
  rather than issuing a fresh root-to-leaf rank query per output position.
  Reports ~50% speedup for full decode vs naive per-position queries; no SIMD;
  no absolute MB/s numbers; no comparison against standard Huffman codecs.
...
produced per second). Their `shuf512` variant on an i9-11900KF (AVX-512,
3.5 GHz, turbo off):
...
This is ~100 MB/s of input on Rocket Lake AVX-512 — construction only,
producing both wire data and rank/select index. Decode throughput is not
measured.
...
- Full text of Baruch/Klein/Shapira 2018 (only summaries fetched so far).
  Specifically: does their full-decode algorithm sneak in a BU pass, or is
  it strictly TD? If TD, pivco's BU/tree_merge stays clearly orthogonal.
